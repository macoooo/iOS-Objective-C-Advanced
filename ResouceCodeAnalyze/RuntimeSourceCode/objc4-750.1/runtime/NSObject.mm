...
struct SideTable {
    spinlock_t slock;
    RefcountMap refcnts;//refcents是一个hash map，其key是obj的地址，而value，则是obj对象的引用计数。
...
template <HaveOld haveOld, HaveNew haveNew,
          CrashIfDeallocating crashIfDeallocating>//使用template进行常量参数传递是为了优化性能
static id 
storeWeak(id *location, objc_object *newObj)//更新指针指向，创建对应的弱引用表
{
 ...
 retry:
    ...
    //防止弱引用间死锁，
    //并且通过+initiaalize初始化构造器保证所有弱引用的isa非空指向
...
{
    //HaveOld: true - 变量有值 - false 需要被及时清理，当前值可能为nil
...
objc_initWeak(id *location, id newObj)
{
    ...
    //enum HaveOld { DontHaveOld = false, DoHaveOld = true };三个bool值
...
#   define POOL_BOUNDARY nil
    ...
    static AutoreleasePoolPage *pageForPointer(uintptr_t p) 
    {
        ...
        // 偏移量
        uintptr_t offset = p % SIZE;//取模获得偏移量
