...
struct method_t {
    SEL name;//方法|函数名，选择器，底层结构跟char*类似
    const char *types;//编码（返回值类型，参数类型,@encode的指令将具体的类型表示成字符串编码）
 ...
 public:
    ...
    void attachLists(List* const * addedLists, uint32_t addedCount) {
        ...
        if (hasArray()) {
            ...
            //array()原来的方法，协议或属性列表数组
            uint32_t oldCount = array()->count;
            uint32_t newCount = oldCount + addedCount;
            setArray((array_t *)realloc(array(), array_t::byteSize(newCount)));
            array()->count = newCount;
            //内存移动 把分类的数据移动到原来数组的第一位
            //将第二个参数的内存复制第三个参数的字节到第一个参数
            //前两个参数内存可重叠
        ...
        //每次方法调用的时候,会在方法列表中查找,而分类的方法在最前头,找到了以后就直接返回了,不会再继续执行!这也是为什么分类和原来类都有相同的方法,却只会调用分类的方法的原因
    }
...
struct class_rw_t {
    ...
    const class_ro_t *ro;//开始类的方法,成员变量 属性 协议等信息都存放在 const class_ro_t中,运行过程中,会将信息整合,动态创建 class_rw_t,然后会将 class_ro_t中的内容(类的原始信息:方法 属性 成员变量 协议信息) 和 分类的方法 属性 协议 成员变量的信息 存储到 class_rw_t中.
    
    
    // 方法信息,二维数组，可读可写，包含了类的初始内容、分类的内容
...
public:
    ...
    class_rw_t* data() {
        ...
        return (class_rw_t *)(bits & FAST_DATA_MASK);//11101000000000000000000000011101111011110111101111011110111101111011110111101111001111111011111
    }
...
// objc_class继承于objc_object,因此类的本质也是个对象
// objc_class中也有isa结构体
struct objc_class : objc_object {
    ...
    //当要调用一个方法时，先不去Class的方法列表中查找，而是先去找cache_t cache
...
struct category_t {
    ...
    method_list_t *methodsForMeta(bool isMeta) {
        if (isMeta) return classMethods;//如果有元类 为类方法列表
