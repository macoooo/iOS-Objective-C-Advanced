...
unattachedCategoriesForClass(Class cls, bool realizing)//取出还未被附加到class上的category list
{
    runtimeLock.assertLocked();
...
attachCategories(Class cls, category_list *cats, bool flush_caches)
{
    ...
    method_list_t **mlists = (method_list_t **)
        malloc(cats->count * sizeof(*mlists));//多个分类中的方法列表
    ...
    free(mlists);
    if (flush_caches  &&  mcount > 0) flushCaches(cls);//刷新方法列表缓存
...
# endif
    ...
    for (EACH_HEADER) {
        category_t **catlist = 
            _getObjc2CategoryList(hi, &count);//获取Category列表
        bool hasClassProperties = hi->info()->hasCategoryClassProperties();//是否有类属性
        ...
        for (i = 0; i < count; i++) {
            ...
            {
                addUnattachedCategoryForClass(cat, cls, hi);//只是给类和category一个关联映射
                if (cls->isRealized()) {
