...
struct weak_entry_t {
    DisguisedPtr<objc_object> referent;//被指对象的地址
    union {
        struct {
            weak_referrer_t *referrers;//可变数组，里面保存着所有指向这个对象的弱引用的地址
            uintptr_t        out_of_line_ness : 2;//标记是否超过内联边界
            uintptr_t        num_refs : PTR_MINUS_2;//数组中已占用的大小
            uintptr_t        mask;//数组下标最大值
            uintptr_t        max_hash_displacement;//最大哈希偏移值
        };
        struct {
            // out_of_line_ness field is low bits of inline_referrers[1]
            weak_referrer_t  inline_referrers[WEAK_INLINE_COUNT];//只有四个元素的数组，默认情况下用它来存储弱引用的指针，大于四个时用上面的*referrers来存，
            //指针数量少，省去哈希
        };
    };
