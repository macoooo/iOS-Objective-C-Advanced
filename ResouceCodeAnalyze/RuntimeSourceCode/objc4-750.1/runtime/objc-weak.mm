...
static void append_referrer(weak_entry_t *entry, objc_object **new_referrer)
{
    if (! entry->out_of_line()) {//如果数组大小没超过4
        // Try to insert inline.
        for (size_t i = 0; i < WEAK_INLINE_COUNT; i++) {
            if (entry->inline_referrers[i] == nil) {
                entry->inline_referrers[i] = new_referrer;//把新的weak指针插到空位置
                return;
            }
        }
        ...
        for (size_t i = 0; i < WEAK_INLINE_COUNT; i++) {
            new_referrers[i] = entry->inline_referrers[i];//将inline中的值赋给referrers
        }
    ...
    while (entry->referrers[index] != nil) {
        ...
        if (index == begin) bad_weak_table(entry);//遍历一圈没找到存储位置，报错
    }
                        ...
                        id *referrer_id)
{
    ...
    if ((entry = weak_entry_for_referent(weak_table, referent))) {//查找referent对象对应的entry
        remove_referrer(entry, referrer);//删除地址为referen的weak 指针
        bool empty = true;
...
weak_clear_no_lock(weak_table_t *weak_table, id referent_id) 
{
    ...
    weak_entry_remove(weak_table, entry);//清除entry
}
